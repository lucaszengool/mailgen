...
class EnhancedEmailSearchAgent {
  ...
  async searchEmails(industry, targetCount = 5, sessionId = null) {
    ...
    try {
      ...
      // 🔥 RAILWAY FIX: Fallback to Scrapingdog API when Python/SearxNG fails
      console.log('🔄 尝试使用Scrapingdog API作为后备搜索方式...');
      try {
        const scrapingdogResult = await this.searchWithScrapingdog(industry, targetCount);
        if (scrapingdogResult.success && scrapingdogResult.prospects.length > 0) {
          console.log(`✅ Scrapingdog后备搜索成功: 找到 ${scrapingdogResult.prospects.length} 个邮箱`);
          return scrapingdogResult;
        }
      } catch (fallbackError) {
        console.error('❌ Scrapingdog后备搜索也失败:', fallbackError.message);
      }
  ...
  /**
   * 🔥 RAILWAY FALLBACK: Use Scrapingdog API directly when Python script fails
   */
  async searchWithScrapingdog(industry, targetCount = 5) {
    const axios = require('axios');
    console.log(`🐕 使用Scrapingdog API搜索: ${industry}`);

    try {
      // Build search query for email discovery
      const searchQuery = `${industry} company email contact`;

      const response = await axios.get('https://api.scrapingdog.com/google', {
        params: {
          api_key: this.apiKey,
          query: searchQuery,
          results: Math.min(targetCount * 3, 20) // Get more results to find emails
        },
        timeout: 30000
      });

      const results = response.data?.organic_results || response.data || [];
      console.log(`📊 Scrapingdog返回 ${results.length} 个搜索结果`);
      ...
      for (const result of results) {
        ...
        for (const email of emails) {
          ...
          // Skip already found emails and generic emails
          if (foundEmails.has(emailLower)) continue;
          if (this.isBlacklistedDomain(emailLower)) continue;
          if (!this.isValidEmailFormat(emailLower)) continue;

          foundEmails.add(emailLower);

          const domain = emailLower.split('@')[1];
          const name = this.extractNameFromEmail(emailLower);
          ...
          prospects.push({
            ...
            confidence: 0.7,
            discoveryMethod: 'google_search_scraping',
            title: this.estimateRole(name, domain),
            department: this.estimateDepartment(name, domain),
            isPersonalEmail: !['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com'].includes(domain)
          });
      ...
      // If no emails found in snippets, try to scrape contact pages
      if (prospects.length === 0) {
        console.log('⚠️ 搜索结果中未直接找到邮箱，尝试搜索联系页面...');
        const contactQuery = `${industry} "contact us" email`;

        const contactResponse = await axios.get('https://api.scrapingdog.com/google', {
          params: {
            api_key: this.apiKey,
            query: contactQuery,
            results: 10
        ...
        const contactResults = contactResponse.data?.organic_results || contactResponse.data || [];
        ...
        for (const result of contactResults) {
          ...
          for (const email of emails) {
            ...
            prospects.push({
              ...
              company: this.extractCompanyFromDomain(domain),
              domain: domain,
              source: 'scrapingdog_contact_page',
              sourceUrl: result.link || result.url || '',
              sourceTitle: result.title || '',
              confidence: 0.6,
              discoveryMethod: 'contact_page_search',
              title: this.estimateRole(name, domain),
              department: this.estimateDepartment(name, domain),
      ...
      console.log(`✅ Scrapingdog搜索完成: 找到 ${prospects.length} 个邮箱`);
      ...
      return {
        ...
        method: 'scrapingdog_api_fallback',
        industry: industry
      };

    } catch (error) {
      console.error('❌ Scrapingdog API调用失败:', error.message);
      throw error;
    }
  }
